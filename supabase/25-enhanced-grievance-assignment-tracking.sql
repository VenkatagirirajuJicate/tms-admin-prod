-- Enhanced Grievance Assignment and Activity Tracking System
-- This SQL file creates all necessary tables, functions, and triggers
-- for comprehensive grievance assignment and activity tracking

-- ==============================================
-- ENUMS AND TYPES
-- ==============================================

-- Activity types enum (extended)
DO $$ BEGIN
...
-- Grievance Activity Log table (enhanced)
...
-- Admin Activity Summary table for performance tracking
...
-- Admin Staff Skills and Specializations table
...
-- Grievance Assignment History table (enhanced)
CREATE TABLE IF NOT EXISTS grievance_assignment_history (
  ...
  assignment_type VARCHAR(50) DEFAULT 'manual', -- manual, automatic, escalated
...
-- Student Activity Visibility table
CREATE TABLE IF NOT EXISTS student_activity_visibility (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  student_id UUID REFERENCES students(id) ON DELETE CASCADE,
  grievance_id UUID REFERENCES grievances(id) ON DELETE CASCADE,
  activity_preferences JSONB DEFAULT '{"show_assignments": true, "show_status_changes": true, "show_admin_actions": true, "show_internal_notes": false}',
...
-- Admin activity summary indexes
CREATE INDEX IF NOT EXISTS idx_admin_activity_summary_admin ON admin_activity_summary(admin_id);
CREATE INDEX IF NOT EXISTS idx_admin_activity_summary_date ON admin_activity_summary(date);

-- Staff skills indexes
CREATE INDEX IF NOT EXISTS idx_admin_staff_skills_admin ON admin_staff_skills(admin_id);
CREATE INDEX IF NOT EXISTS idx_admin_staff_skills_category ON admin_staff_skills(skill_category);
...
CREATE INDEX IF NOT EXISTS idx_grievance_assignment_history_assigned_to ON grievance_assignment_history(assigned_to);
CREATE INDEX IF NOT EXISTS idx_grievance_assignment_history_active ON grievance_assignment_history(is_active);

-- ==============================================
-- FUNCTIONS
-- ==============================================

-- Enhanced function to log grievance activities
...
-- Insert default admin staff skills for existing admins
INSERT INTO admin_staff_skills (admin_id, skill_category, specialization_areas, max_concurrent_cases, preferred_case_types)
SELECT 
  au.id,
  'general',
  CASE 
...
ALTER TABLE admin_activity_summary ENABLE ROW LEVEL SECURITY;
ALTER TABLE admin_staff_skills ENABLE ROW LEVEL SECURITY;
ALTER TABLE grievance_assignment_history ENABLE ROW LEVEL SECURITY;
ALTER TABLE student_activity_visibility ENABLE ROW LEVEL SECURITY;

-- Activity log policies
CREATE POLICY "Students can view public activities for their grievances"
...
-- Admin activity summary policies
CREATE POLICY "Admins can view activity summaries"
ON admin_activity_summary FOR SELECT
USING (
  EXISTS (
    SELECT 1 FROM admin_users 
    WHERE email = auth.jwt() ->> 'email' 
...
ON admin_activity_summary FOR ALL
USING (true);

-- Staff skills policies
CREATE POLICY "Admins can view staff skills"
ON admin_staff_skills FOR SELECT
USING (
  EXISTS (
    SELECT 1 FROM admin_users 
    WHERE email = auth.jwt() ->> 'email' 
...
CREATE POLICY "Students can view assignment history for their grievances"
ON grievance_assignment_history FOR SELECT
USING (
  grievance_id IN (
    SELECT id FROM grievances 
    WHERE student_id IN (
...
ON grievance_assignment_history FOR ALL
USING (
  EXISTS (
    SELECT 1 FROM admin_users 
    WHERE email = auth.jwt() ->> 'email' 
...
COMMENT ON TABLE admin_activity_summary IS 'Daily summary of admin activities for performance tracking and reporting';
COMMENT ON TABLE admin_staff_skills IS 'Admin staff skills, specializations, and capacity management';
COMMENT ON TABLE grievance_assignment_history IS 'Complete history of grievance assignments including reassignments';
COMMENT ON TABLE student_activity_visibility IS 'Student preferences for activity visibility and notifications';
...
COMMENT ON FUNCTION get_available_admin_staff IS 'Returns available admin staff with workload and performance metrics';
COMMENT ON FUNCTION get_grievance_activity_timeline IS 'Returns public activity timeline for student view';
COMMENT ON FUNCTION get_recommended_admin_for_grievance IS 'Recommends best admin for grievance assignment based on multiple criteria'; 